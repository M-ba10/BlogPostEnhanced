...
def inspect_connection_string():
    ...
    uri = "postgresql://flask_user:aqw123@localhost:5432/blog_flask_prod"
...
def get_database_uri():
    if 'DATABASE_URL' in os.environ:
        ...
        return f"{uri}?options=-c%20lc_messages%3DC"  # uri

    return "postgresql://flask_user:aqw123@localhost:5432/blog_flask_prod?options=-c%20lc_messages%3DC"


# return "postgresql://flask_user:aqw123@localhost:5432/blog_flask_prod"
...
def create_app():
    ...
    # Set up database connection with explicit handling
    uri = get_database_uri()
    app.config['SQLALCHEMY_DATABASE_URI'] = uri
    app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

    # Add explicit engine options
    app.config['SQLALCHEMY_ENGINE_OPTIONS'] = {
        "connect_args": {
            "options": "-c lc_messages=C",
            # Try specifying client_encoding explicitly
    ...
    class Like(db.Model):
        ...
        _table_args_ = (db.UniqueConstraint('user_id', 'post_id', name='unique_user_post_like'),)
...
with app.app_context():
    try:
        ...
        result = connection.execute("SHOW client_encoding;").fetchone()
        print(f"Database client encoding: {result[0]}")

        result = connection.execute("SHOW server_encoding;").fetchone()
        print(f"Database server encoding: {result[0]}")

        connection.close()
        db.create_all()
    except UnicodeDecodeError as e:
        print(f"❌ Unicode error in database connection: {str(e)}")
        print(f"Problematic byte: {hex(e.object[e.start])}")
        print("Check your password and other parameters for non-ASCII characters.")
        character_position = e.start
        context_start = max(0, character_position - 10)
        context_end = min(len(e.object), character_position + 10)
        print(f"Context around error (as hex): {e.object[context_start:context_end].hex()}")
